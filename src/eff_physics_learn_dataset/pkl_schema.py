...
def _infer_solutions(raw: Mapping[str, Any]) -> np.ndarray:
    ...
    chosen_key = arrays[0][0]
    warnings.warn(
        f"pkl_schema: no 'solutions' or 'u' key found; falling back to largest array "
        f"(key={chosen_key!r}, shape={arrays[0][1].shape}). "
        "Verify this is the correct solutions array.",
        UserWarning,
        stacklevel=2,
    )
    return arrays[0][1]
