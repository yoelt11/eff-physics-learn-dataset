...
def finite_diff_1d(u, dx: float, order: int = 2, axis: int = -1):
    """Compute first derivative using centered finite differences.

    Args:
        u: Array to differentiate (jax or numpy)
        dx: Grid spacing
...
def apply_laplacian_2d_fd(u, dx: float, dy: float):
    """Apply 2D Laplacian using finite differences.

    Args:
        u: 2D array (nx, ny) — jax or numpy array
        dx: x grid spacing
        dy: y grid spacing
